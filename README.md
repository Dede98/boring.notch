...
- **Optional, for AI usage stats**: Install and run [`claude-usage`](https://github.com/Dede98/claude-usage). The Claude/OpenAI usage widget reads the usage data generated by this tool, so the widget will not show live provider limits unless `claude-usage` is installed and its daemon/data export is working.
...
You can also build the app without opening Xcode:

```bash
xcodebuild -project boringNotch.xcodeproj -scheme boringNotch -configuration Debug -destination 'platform=macOS' build
```

The Debug app bundle is written to Xcode DerivedData, typically at:

```bash
~/Library/Developer/Xcode/DerivedData/boringNotch-*/Build/Products/Debug/boringNotch.app
```

If you are iterating locally and do not see changes after rebuilding, fully quit the already-running menu-bar app before reopening the new Debug build. macOS may otherwise keep the existing `LSUIElement` process alive.

### AI Usage Widget Dependency

This fork includes a compact Claude/OpenAI usage widget for the notch. It depends on [`claude-usage`](https://github.com/Dede98/claude-usage) to collect and expose provider usage data.

After installing `claude-usage`, start its daemon/export flow as documented in that repository. Boring Notch expects the generated usage data to be available in the user-level Claude usage data location, for example:

```bash
~/.claude/usage-data.json
```

If that file is missing or stale, the widget will show an empty or stale state instead of current limits.
